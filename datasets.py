...
def dirichlet_nonIID_data(train_data, conf):
    ...
    plt.hist([labels[idc] for idc in client_idcs], stacked=True,
             ...
             rwidth=0.8)
    plt.xticks(np.arange(n_classes), fontsize=35)
    plt.yticks(fontsize=35)
    # plt.xticks(np.array([0, 9, 19, 29, 39, 49, 59, 69, 79, 89, 99]), fontsize=20)
    plt.xlabel("Labels", fontsize=35)
    plt.ylabel("Samples", fontsize=35)
    plt.legend(loc="upper right", fontsize=20)
    plt.title("CIFAR10", fontsize=35)
    plt.show()
...
if __name__ == "__main__":
    ...
    train_datasets, eval_datasets = get_dataset("../data/cifar10/", "cifar10")
